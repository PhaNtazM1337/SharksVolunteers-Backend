...
const roles = {
  ...
  async getTeacher(ctx) {
    let form = ctx.request.body;
    let result = retCode.Success;
    let bkdata = await model.getTeacher(form);
	result.data = bkdata[0].isTeacher;
    return com.filterReturn(result);
  },
  
  async regTeacher(ctx) {
    let form = ctx.request.body;
    let result = retCode.Success;
  ...
  async getList(ctx) {
    ...
    ctx.request.body.wheres =
      " helplist.wx_id = wxuser.id and helplist.is_delete = 0 and helplist.state in (1,2,3) and helplist.title in ('Need Help','上门维修','代替服务','其他帮助') and helplist.a_id=" + ctx.request.body.a_id;
